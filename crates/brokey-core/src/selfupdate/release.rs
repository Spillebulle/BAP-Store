...
pub const REPOSITORY: &str = "https://github.com/Spillebulle/Brokey";

/// Where somebody goes to fetch a build by hand, which is the answer for
/// every installation Brokey may not update itself.
pub const RELEASES_PAGE: &str = "https://github.com/Spillebulle/Brokey/releases";

/// The one request this module makes.
pub const API_LATEST: &str = "https://api.github.com/repos/Spillebulle/Brokey/releases/latest";
...
impl Asset {
    /// Whether this is an address Brokey is willing to fetch from.
    ///
...
pub struct Release {
    ...
    /// release with no title at all is called "Brokey <v>" here.
    pub name: String,
...
pub fn parse(json: &str) -> Result<Option<Release>> {
    ...
    Ok(Some(Release {
        name: raw
            .name
            .filter(|n| !n.is_empty())
            .unwrap_or_else(|| format!("Brokey {version}")),
...
fn describe_failure(e: &Error) -> Error {
    if is_rate_limited(e) {
        return Error::new(
            "GitHub's request limit for this address is used up, so a newer Brokey could not be checked for. Try again in an hour.",
        );
    }
    Error::new(format!(
        "Could not check GitHub for a newer Brokey: {}. Try again later.",
        e.message
    ))
}
...
mod tests {
    ...
    fn a_body_that_is_absent_reads_as_empty_rather_than_failing() {
        ...
        assert_eq!(release.name, "Brokey 1.0.0");
    ...
    fn a_403_or_429_names_the_request_limit_and_when_to_try_again() {
        ...
        assert_eq!(
            e.message,
            "Could not check GitHub for a newer Brokey: could not reach api.github.com. Try again later."
        );
        assert!(

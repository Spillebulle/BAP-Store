//! The two self-update commands' view of `brokey_core::selfupdate`: the check,
...
use brokey_core::selfupdate::{Probe, Version, assemble, detect};
use brokey_core::{Plan, Step};

pub use brokey_core::selfupdate::SelfUpdate;
...
/// Ask whether a newer Brokey exists and how this copy would get it.
...
pub fn check(fresh: bool) -> Result<SelfUpdate, String> {
    Ok(brokey_core::selfupdate::check(
        &Client::shared(),
        &Probe::current(),
        fresh,
    ))
...
pub fn plan(update: &SelfUpdate) -> Result<Plan, String> {
    let Some(remedy) = update.remedy.as_ref() else {
        return Err(match &update.latest {
            Some(latest) if latest.newer => "A newer Brokey exists but this copy cannot be updated from inside the application. The check says where to get it.".to_string(),
            _ => "Brokey is up to date. There is nothing to apply.".to_string(),
        });
    };
    let steps = brokey_core::selfupdate::plan(remedy, &brokey_core::selfupdate::download_dir())
        .map_err(|e| e.message)?;
    Ok(wrap(steps))
}
...
pub fn wrap(steps: Vec<Step>) -> Plan {
    Plan {
        id: brokey_core::transaction::plan::new_id(),
        ops: Vec::new(),
        steps,
    }
}

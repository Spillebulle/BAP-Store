...
fn debian() -> SystemInfo {
    bap_core::system::from_os_release("ID=debian\n")
}
...
impl Transport for Canned {
    ...
    fn get(&self, path: &str) -> bap_core::Result<Response> {
        if self.unreachable.contains(&path) {
            return Err(bap_core::Error::from_source(
                SourceKind::Snap,
...
fn live_snapd_answers_system_info_and_lists_what_is_installed() {
    ...
    let s = Snap::new(
        &bap_core::system::detect(),
        bap_core::http::Client::shared(),
    );
    let status = s.status();

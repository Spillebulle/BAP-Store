...
use bap_core::sources::fwupd::{self, Fwupd};
use bap_core::{Op, PackageKind, PackageRef, Query, Source, SourceKind};
...
fn plans_that_need_no_tool() {
    let source = Fwupd::new(&bap_core::system::from_os_release(""));
...
fn live_fwupd_lists_this_machines_devices() {
    let source = Fwupd::new(&bap_core::system::detect());

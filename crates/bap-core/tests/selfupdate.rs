...
use bap_core::model::SourceKind;
use bap_core::selfupdate::{
...
fn every_installation_is_detected_from_its_own_readings() {
    ...
    let appimage = [
        ("HOME", "/home/a"),
        ("APPIMAGE", "/home/a/Apps/BAP-Store-0.1.0-x86_64.AppImage"),
    ];
    let flatpak = [
        ("HOME", "/home/a"),
        ("FLATPAK_ID", "io.github.spillebulle.bapstore"),
    ];
    let cases: Vec<Case<'_>> = vec![
        ...
        (
            "appimage",
            "/tmp/.mount_BAPSto1a/usr/bin/bap-store",
            &appimage,
            &["/home/a/Apps/BAP-Store-0.1.0-x86_64.AppImage"],
            Installation::AppImage {
                path: PathBuf::from("/home/a/Apps/BAP-Store-0.1.0-x86_64.AppImage"),
            },
        ),
        ...
        (
            ...
            "/usr/bin/bap-store",
            &home,
            &["/var/lib/pacman", "/var/lib/pacman/local/bap-store-*"],
            Installation::Pacman {
        ...
        (
            ...
            &[
                "/var/lib/pacman",
                "/var/lib/pacman/local/bap-store-*",
                "/var/lib/pacman/local/bap-store-bin-*",
            ],
            Installation::Pacman {
                package: "bap-store-bin".into(),
            },
        ),
        (
        ...
        (
            "portable under home",
            "/home/a/bap-store-0.1.0/bap-store",
            &home,
            &["/var/lib/pacman", "/var/lib/pacman/local/bap-store-*"],
...
fn the_fixture_reads_as_the_release_it_is() {
    ...
    assert_eq!(r.name, "BAP Store 0.2.0");
    assert_eq!(
        r.url,
        "https://github.com/Spillebulle/BAP-Store/releases/tag/v0.2.0"
    );
...
fn every_documented_asset_name_is_in_the_fixture_and_is_found() {
    ...
    let expected = [
        (Format::Deb, Arch::X86_64, "bap-store_0.2.0_amd64.deb"),
        (Format::Deb, Arch::Aarch64, "bap-store_0.2.0_arm64.deb"),
        (Format::Rpm, Arch::X86_64, "bap-store-0.2.0-1.x86_64.rpm"),
        (Format::Rpm, Arch::Aarch64, "bap-store-0.2.0-1.aarch64.rpm"),
        (
            Format::PacmanPackage,
            Arch::X86_64,
            "bap-store-bin-0.2.0-1-x86_64.pkg.tar.zst",
        ),
        (
            Format::AppImage,
            Arch::X86_64,
            "BAP-Store-0.2.0-x86_64.AppImage",
        ),
        (
            Format::AppImage,
            Arch::Aarch64,
            "BAP-Store-0.2.0-aarch64.AppImage",
        ),
        (
            Format::Flatpak,
            Arch::X86_64,
            "bap-store-0.2.0-x86_64.flatpak",
        ),
    ];
    for (format, arch, name) in expected {
        assert_eq!(format.name(version, arch), name);
        let found = bap_core::selfupdate::remedy::find(&r.assets, format, Some(arch))
            .unwrap_or_else(|| panic!("{format:?} {arch:?} not found in the fixture"));
        assert_eq!(found.name, name);
    }
    // The ones the table says are not built.
    assert_eq!(
        bap_core::selfupdate::remedy::find(&r.assets, Format::PacmanPackage, Some(Arch::Aarch64)),
        None
    );
    assert_eq!(
        bap_core::selfupdate::remedy::find(&r.assets, Format::Flatpak, Some(Arch::Aarch64)),
        None
    );
    assert_eq!(
        bap_core::selfupdate::remedy::find(&r.assets, Format::Deb, None),
        None
    );
}
...
fn no_upgrade_command_reaches_a_machine_without_an_archive() {
    const FORBIDDEN: [&str; 5] = [
        ...
        "pacman -Syu bap-store",
        "flatpak update",
        "dnf upgrade bap-store",
        "already the newest",
    ];
...
fn each_installation_gets_the_rule_the_style_guide_settles() {
    ...
    match &r {
        Remedy::InstallAsset {
            ...
            assert_eq!(asset, "bap-store-bin-0.2.0-1-x86_64.pkg.tar.zst");
            assert!(url.ends_with("/bap-store-bin-0.2.0-1-x86_64.pkg.tar.zst"));
    ...
    assert!(
        matches!(&r, Remedy::InstallAsset { asset, installer: Installer::DpkgI, .. } if asset == "bap-store_0.2.0_amd64.deb"),
        "{r:?}"
    );
    let r = remedy(
        &Installation::Dpkg { archive: false },
    ...
    assert!(
        matches!(&r, Remedy::InstallAsset { asset, installer: Installer::DpkgI, .. } if asset == "bap-store_0.2.0_arm64.deb"),
        "{r:?}"
    );
    let r = remedy(&Installation::Rpm { archive: false }, &latest, x86, &assets);
    assert!(
        matches!(&r, Remedy::InstallAsset { asset, installer: Installer::RpmU, .. } if asset == "bap-store-0.2.0-1.x86_64.rpm"),
        "{r:?}"
    );
    let r = remedy(
        &Installation::Rpm { archive: false },
    ...
    assert!(
        matches!(&r, Remedy::InstallAsset { asset, installer: Installer::RpmU, .. } if asset == "bap-store-0.2.0-1.aarch64.rpm"),
        "{r:?}"
    );
    ...
    assert_eq!(
        ...
        Remedy::Sentence {
            sentence: "This copy runs as a Flatpak bundle with no remote behind it. \
                       Take bap-store-0.2.0-x86_64.flatpak from the releases page and install it over this one."
                .into()
        }
    );
    ...
    let path = PathBuf::from("/home/a/Apps/BAP-Store-0.1.0-x86_64.AppImage");
    let r = remedy(
    ...
    match &r {
        Remedy::ReplaceFile {
            ...
            assert_eq!(p, &path);
            assert_eq!(asset, "BAP-Store-0.2.0-x86_64.AppImage");
    ...
    assert!(
        matches!(&r, Remedy::ReplaceFile { asset, .. } if asset == "BAP-Store-0.2.0-aarch64.AppImage"),
        "{r:?}"
    );
    ...
    assert!(matches!(r, Remedy::Sentence { .. }), "{r:?}");
    assert!(
        r.sentence().contains("BAP-Store-0.2.0-x86_64.AppImage"),
        "{}",
        r.sentence()
    );
...
fn a_release_that_built_x86_only_leaves_arm_with_a_sentence() {
    ...
    for installation in every_installation() {
        ...
        if has_archive(&installation)
            || matches!(installation, Installation::Pacman { ref package } if package == "bap-store")
        {
            assert!(
...
fn a_newer_release_carries_a_remedy_and_an_older_one_does_not() {
    ...
    assert_eq!(s.installation_label, "the bap-store-bin package");
...
fn the_check_serialises_with_lower_case_tags_for_the_page() {
    ...
    let back: bap_core::selfupdate::SelfUpdate = serde_json::from_value(json).expect("round trips");
    assert_eq!(back, s);
}

#[test]
fn an_install_asset_plans_a_download_and_a_root_install() {
    let dir = Path::new("/home/a/.cache/bap-store/http/downloads");
    let cases = [
        (
            Installation::Pacman {
                package: "bap-store-bin".into(),
...
fn a_replace_file_plans_a_download_and_an_install_of_the_file_as_the_user() {
    ...
    let downloaded = dir
        .join("BAP-Store-0.2.0-x86_64.AppImage")
        .display()
        .to_string();
...
fn the_flatpak_bundle_installer_plans_as_the_user_if_ever_used() {
    ...
    let r = Remedy::InstallAsset {
        asset: "bap-store-0.2.0-x86_64.flatpak".into(),
        url: "https://github.com/x/bap-store-0.2.0-x86_64.flatpak".into(),
    ...
    assert_eq!(
        ...
        [
            ...
            "--user",
            "-y",
            "/d/bap-store-0.2.0-x86_64.flatpak"
        ]
    );
...
fn live_latest_release_is_none_or_a_version() {
    ...
    match release::latest(&client, false) {
        ...
        Ok(Some(r)) => {
            ...
            assert!(
                r.url
                    .starts_with("https://github.com/Spillebulle/BAP-Store/")
            );
        }
        Err(e) => panic!("{e}"),
    }

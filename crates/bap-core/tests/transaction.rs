...
use bap_core::model::*;
use bap_core::transaction::allow;
use bap_core::transaction::progress::{Reading, parser_for};
use bap_core::transaction::runner::{self, Outcome, Runner};
...
fn a_session_step_gets_its_working_directory_and_environment() {
    ...
    let mut step = session("Looking around", "sh", &["-c", "echo $BAP_TEST_WORD; pwd"]);
    step.command
        .env
        .push(("BAP_TEST_WORD".to_string(), "hello".to_string()));
...
fn a_program_that_cannot_start_fails_its_step_with_a_sentence() {
    let plan = plan(vec![session("Never", "definitely-not-a-program-bap", &[])]);
    let (outcome, events) = run_collect(&Runner::new().with_helper(None), &plan);
    assert!(!outcome.ok);
    assert!(
        outcome
            .message
            .starts_with("Never failed. Could not start definitely-not-a-program-bap: "),
        "{}",
        outcome.message
    );
...
fn root_steps_without_a_helper_fail_before_anything_runs() {
    ...
    for helper in [None, Some(PathBuf::from("/nonexistent/bap-helper"))] {
...
/// with a note otherwise, because `cargo test -p bap-core` does not build
/// `bap-helper` and must not depend on it.
#[test]
fn the_built_helper_checks_plans_and_refuses_to_run_as_a_user() {
    let Some(helper) = runner::locate_helper() else {
        eprintln!("skipped: no bap-helper build found; run `cargo build -p bap-helper` first");
        return;
    };
    let version = Process::new(&helper).arg("--version").output().unwrap();
    if !version.stdout.starts_with(b"bap-helper ") {

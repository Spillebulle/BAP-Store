...
use bap_core::sources::github::{self, Arch, AssetKind, Github, InstallRecord, InstallTarget};
...
fn plans_per_asset_kind() {
    let release = github::parse_release(HEROIC).unwrap();
    let downloads = Path::new("/home/me/.cache/bap-store/downloads");
    ...
    let path = "/home/me/.cache/bap-store/downloads/Heroic-2.22.1-linux-x64.pacman";
    assert_eq!(
    ...
    assert_eq!(
        ...
        vec![
            "-Dm755",
            "/home/me/.cache/bap-store/downloads/Heroic-2.22.1-linux-x86_64.AppImage",
...
fn live_github_search_finds_heroic() {
    let dir = tempfile::tempdir().unwrap();
    let source = source(&bap_core::system::detect(), dir.path());

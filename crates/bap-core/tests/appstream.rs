...
//! `BAP_APPSTREAM_DIR` at an extracted copy (a directory holding `xml/` and
//! `icons/`) and run `cargo test -p bap-core -- --ignored live_`.

use bap_core::Picture;
use bap_core::appstream::{Catalogue, extra_roots};
...
fn ids<'a>(components: &[&'a bap_core::appstream::Component]) -> Vec<&'a str> {
...
#[test]
fn bap_appstream_dir_names_roots_that_load_like_the_system_ones() {
...
/// `BAP_APPSTREAM_DIR` pointing at an extracted copy of it.
#[test]
#[ignore]
fn live_the_distribution_catalogue_loads_with_icons_on_disk() {
    ...
    println!(
        "{} components; first load {first:?}, second load (memoised) {second:?}; BAP_APPSTREAM_DIR={:?}",
        catalogue.len(),
        std::env::var("BAP_APPSTREAM_DIR").ok()
    );
    assert!(
        catalogue.len() > 1000,
        "only {} components: install the AppStream catalogue package or set BAP_APPSTREAM_DIR to an extracted copy",
        catalogue.len()
    );

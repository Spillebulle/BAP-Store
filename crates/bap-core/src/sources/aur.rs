...
//! elevated, through `pkexec`. Running the whole build through `bap-helper`
...
# Written by BAP Store before every AUR build. Edits here are lost.
...
pub fn is_self(name: &str) -> bool {
    name == "bap-store" || name == "bap-store-bin"
}

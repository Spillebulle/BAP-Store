...
//! `bap-helper` instead would make flatpak run as root and put the user's
...
const SELF_APP_ID: &str = "io.github.spillebulle.bapstore";
...
pub fn parse_remotes(text: &str, asked: Installation) -> Vec<Remote> {
    ...
    for line in text.lines() {
        ...
        if let Some(name) = foreign {
            log::info!(
                "skipping remote {} in installation {name}, which BAP Store does not manage",
                cells[0]
            );
            continue;
        }

...
pub fn notice_for(label: &str, export_dirs: &[PathBuf], session: &[PathBuf]) -> Option<String> {
    ...
    unseen.then(|| {
        format!(
            "{label} applications are installed but this desktop session started before {label} was set up, so your launcher does not list them yet. Log out and back in once to see them there; until then, open them from BAP Store."
        )
    })
}

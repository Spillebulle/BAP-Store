...
//! `BAP_APPSTREAM_DIR` adds extra roots: a colon-separated list of
...
//!
//! ```sh
//! BAP_APPSTREAM_DIR=/tmp/asd/usr/share/swcatalog bap-store search steam
//! ```
//!
...
impl Catalogue {
    ...
    pub fn load_system(_system: &SystemInfo) -> Arc<Catalogue> {
        let mut dirs = extra_roots(std::env::var("BAP_APPSTREAM_DIR").ok().as_deref())
            .into_iter()
...
/// The extra catalogue roots named by `BAP_APPSTREAM_DIR`: colon-separated,

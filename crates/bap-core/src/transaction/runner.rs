//! Executes a plan. Root steps go to `bap-helper` through `pkexec`, one
...
pub const HELPER_MISSING: &str =
    "The helper that installs packages was not found. Reinstall BAP Store.";
...
pub const HELPER_PATHS: [&str; 2] = [
    "/usr/lib/bap-store/bap-helper",
    "/usr/libexec/bap-store/bap-helper",
];
...
/// Where the helper is on this machine: `BAP_HELPER` for development, then
...
pub fn locate_helper() -> Option<PathBuf> {
    if let Some(path) = std::env::var_os("BAP_HELPER").map(PathBuf::from)
        && path.is_file()
    {
        return Some(path);
    }
    ...
    {
        let beside = dir.join("bap-helper");
        if beside.is_file() {
            return Some(beside);
        }
        ...
        {
            let above = parent.join("bap-helper");
            if above.is_file() {
                return Some(above);
            }
        }

//! How this copy of BAP Store was installed, and therefore whether it may
//! replace itself.
//!
//! BAP Store ships in six shapes, and they divide into two groups that are
//! treated completely differently:
//!
//! * **The ones BAP Store owns.** An AppImage, which is a single file, and a
...
/// The package names this project publishes to pacman. `bap-store` builds
/// from source in the AUR; `bap-store-bin` is the release asset.
pub const PACMAN_SOURCE_PACKAGE: &str = "bap-store";
pub const PACMAN_BIN_PACKAGE: &str = "bap-store-bin";
...
pub enum Installation {
    ...
    /// pacman's package: `bap-store` from the AUR, which the AUR source
    /// reports as an ordinary update, or `bap-store-bin`, which is installed
    ...
    /// `/usr/local`. BAP Store may replace the binary.
    Portable,
...
impl Installation {
    ...
    /// Whether BAP Store may write the new binary itself, without a package
    /// manager in between.
...
/// component (`/var/lib/pacman/local/bap-store-bin-*`) meaning "any entry
...
pub fn detect(probe: &Probe) -> Installation {
    ...
    // sandbox the executable is at /app/bin/bap-store, a system path like
    ...
    if probe.exists("/var/lib/pacman") {
        // `bap-store-*` also matches `bap-store-bin-*`, so the bin package
        // is tested first.
...
mod tests {
    ...
    fn the_wildcard_matches_a_prefix_in_a_real_directory() {
        ...
        std::fs::create_dir(dir.path().join("bap-store-bin-0.1.0-1")).expect("mkdir");
        std::fs::create_dir(dir.path().join("bap-helper-0.1.0-1")).expect("mkdir");
        assert!(path_exists(&dir.path().join("bap-store-bin-*")));
        assert!(path_exists(&dir.path().join("bap-store-*")));
        assert!(!path_exists(&dir.path().join("bap-store-git-*")));
        assert!(!path_exists(&dir.path().join("nothing-*")));
        // Without the star it is a plain existence test.
        assert!(path_exists(&dir.path().join("bap-helper-0.1.0-1")));
        assert!(!path_exists(&dir.path().join("bap-helper")));
    ...
    fn the_id_is_what_serde_writes() {
        for installation in [
            ...
            Installation::AppImage {
                path: PathBuf::from("/home/a/BAP-Store.AppImage"),
            },
            Installation::Pacman {

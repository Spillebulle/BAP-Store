...
//! left out because BAP Store's README does not carry a download table yet.

use std::path::{Path, PathBuf};

fn repo_root() -> PathBuf {
    // CARGO_MANIFEST_DIR is crates/bap-store.
...
fn the_tauri_config_carries_this_version() {
    let version = env!("CARGO_PKG_VERSION");
    let found = json_version("crates/bap-store/tauri.conf.json");
    assert_eq!(
        found, version,
        "crates/bap-store/tauri.conf.json says `{found}` but this build is `{version}`. \

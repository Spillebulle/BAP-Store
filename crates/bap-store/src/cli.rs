//! Text mode: `bap-store search steam`, `bap-store sources`, `bap-store
...
pub const USAGE: &str = "\
Usage: bap-store <command> [options]
...
Without a command, bap-store opens the window.
...
fn usage_error(message: &str) -> i32 {
    eprintln!("bap-store: {message} Run bap-store help to see the commands.");
    2
}
...
/// term, so `bap-store search visual studio code` asks what a user typing
...
pub fn parse_search(args: &[String]) -> Result<SearchArgs, String> {
    ...
    if term.is_empty() {
        return Err("search needs a term, for example bap-store search steam.".to_string());
    }
    Ok(SearchArgs {
        term,
        sources,
...
fn search(args: &[String]) -> i32 {
    ...
    if result.searched.is_empty() {
        println!("No source is available to search. Run bap-store sources to see why.");
...
fn drivers(args: &[String]) -> i32 {
    ...
    let store = Store::detect();
    let report = bap_core::drivers::report(&store);

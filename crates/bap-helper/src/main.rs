//! The one part of BAP Store that runs as root. Started as
//! `pkexec bap-helper run`, it reads one JSON plan on the first line of
//! stdin, checks every step against the closed list in
//! `bap_core::transaction::allow` before running any, runs them one after
...
//! is; the runner in `bap-core` does the thinking, and this binary stays
...
use bap_core::transaction::allow::{self, Allowed};
use bap_core::{Event, Plan, Step};
...
const USAGE: &str = "Usage: bap-helper run | check | --version. \
...
fn main() {
    ...
    let code = match args.first().map(String::as_str) {
        ...
        Some("check") => check(),
        Some("--version") | Some("-V") => {
            println!("bap-helper {}", env!("CARGO_PKG_VERSION"));
            EXIT_OK
        }
        _ => {
            eprintln!("{USAGE}");
...
fn run() -> i32 {
    if geteuid() != 0 {
        eprintln!(
            "bap-helper run only works as root and is started through pkexec by BAP Store. It refuses to run as an ordinary user."
        );
...
/// system caches plus the invoking user's `~/.cache/bap-store`. Under
